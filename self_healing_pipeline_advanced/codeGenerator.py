...
def llm_generate_scripts(state: AgentState) -> AgentState:
    """
    Ask the LLM to produce 5 failing variants of the Spark app.
    Each variant is generated by calling the LLM separately.
    """
    scripts = []
    ...
    for i, failure_desc in enumerate(failure_descriptions, start=1):
        prompt = [
            SystemMessage(
                content=(
                    ...
                    f"- The failure should be unique: {failure_desc}\n"
                    "- Preserve all Python indentation exactly.\n"
                    "- Keep the main logic, table writes, merges, and generator structure intact.\n"
                    "- Do NOT introduce artificial syntax errors or nonsense lines.\n"
                    "- Return ONLY the full Python code, no markdown or explanations.\n\n"
                    f"BASE TEMPLATE:\n{SPARK_APP_TEMPLATE}"
                )
            )
        ]
        ...
        # Preserve indentation, just trim top/bottom blank lines
