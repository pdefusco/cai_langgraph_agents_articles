    ...
    prompt = [
        SystemMessage(
            content=(
                ...
                "5. Preserve all Python indentation.\n"
                "6. Work with the same tables and try to achieve the same goal, but do change anything needed in the logic to figure out what is the issue and to make the updated script run and succeed.\n"
                "7. If you find the error, do not include logic to prevent the original app to complete by breaking early."
                "8. If you find an issue with a join or merge key, update the code to include logic to read the schemas of both source and target tables and infer from those schemas which columns or statements should be updated"
                "9. Do not add a How to Run or any other comments and docstrings at the end of the script.\n"
                "10. Do not add any markdown ticks.\n"
                "11. Assume the spark submit will be the same, there is no modification to any of its options and script arguments.\n"
                "12. Single line comments showing changes in the code are ok, as long as each line is properly formatted starting with a #"
